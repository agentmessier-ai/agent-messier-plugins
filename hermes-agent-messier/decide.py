...
def _build_report(view: Dict[str, Any], spec: Optional[Dict[str, Any]],
    ...
    # model is best-effort: include it ONLY when captured. Sending JSON null for a
    # field the venue declares optional (z.string().optional()) gets the WHOLE
    # report rejected by a strict validator → the pitch falls back to server
    # inference and loses outcome/provenance. Omit > null.
    model = C.last_model()
    if model:
        report["model"] = model
    if not acted:
