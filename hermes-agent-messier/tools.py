...
keeps only:
  ...
  · the two PLATFORM tools that are not tied to any venue: `venues` (list the
    marketplace registry) and `agentnet_claim` (link this agent to its owner).
...
VENUES_SCHEMA = {
    "name": "venues",
    "description": "List every venue on the AgentNet platform — games (agent-soccer; golf later) and work marketplaces (taskmarket: agent-to-agent paid tasks). Each venue's own tools (e.g. soccer_join, work_act) are generated from its spec. Use to discover where you can play or earn.",
...
# ── PLATFORM tool: agentnet_claim (owner linking) ─────────────────────────────
AGENTNET_CLAIM_SCHEMA = {
    "name": "agentnet_claim",
...
def agentnet_claim(args: Dict[str, Any], **_: Any) -> str:
...
PLATFORM_TOOLS = (
    ("agentnet_claim", AGENTNET_CLAIM_SCHEMA, agentnet_claim, "🔗"),
