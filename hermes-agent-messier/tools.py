...
# The plugin is a PLATFORM client: venues (games AND work markets) are
# discovered from the registry; each venue's spec publishes its actions,
# instructions and ROUTE TEMPLATES — so acting on a venue needs zero
# venue-specific code here.

_VENUE_DEFAULTS = {"taskmarket": "http://localhost:3030"}
_venue_specs: Dict[str, Dict[str, Any]] = {}


def _reset_venue_cache() -> None:
    _venue_specs.clear()


def _venue_url(origin: str) -> str:
    """Resolve a registry `origin` to a base URL: full URLs pass through;
    'pitch' is the platform itself; otherwise AGENTNET_<ORIGIN>_URL env, then
    the dev default."""
    ...
    if origin == "pitch":
        return C.server_url()
    import os as _os
    env = _os.getenv(f"AGENTNET_{origin.upper()}_URL")
    return env or _VENUE_DEFAULTS.get(origin, C.server_url())


def _venue_spec(origin: str) -> Optional[Dict[str, Any]]:
    """The venue's spec, cached per process. Ladder: cached → fetch → None."""
    if origin in _venue_specs:
        return _venue_specs[origin]
    try:
        spec = C.request("GET", "/spec", base=_venue_url(origin))
    except C.PitchError:
        return None
    if isinstance(spec, dict) and spec.get("actions"):
        _venue_specs[origin] = spec
        return spec
    return None
...
WORK_OBSERVE_SCHEMA = {
    "name": "work_observe",
    "description": "Look at the task marketplace from your point of view: new events since your last look, tasks you posted (with bids), tasks you're working, and what's open for bidding — plus a summary and per-task legalActions. Call before work_act.",
    "parameters": {"type": "object", "properties": {
        "cursor": {"type": "number", "description": "event cursor from your previous observe (default 0)"},
    }},
}


def work_observe(args: Dict[str, Any], **_: Any) -> str:
    spec = _venue_spec("taskmarket")
    if not spec:
        return _err("taskmarket venue unreachable — try again later")
    did = C.team_handle()
    path = str(spec.get("routes", {}).get("observe", "/agents/{did}/observe")).replace("{did}", did)
    cursor = int(args.get("cursor") or 0)
    try:
        obs = C.request("GET", f"{path}?cursor={cursor}", base=_venue_url("taskmarket"), caller_did=did)
    except C.PitchError as e:
        return _err(e.message, status=e.status)
    return _ok({"ok": True, **obs,
                "hint": "act with work_act (action + params); only actions in a task's legalActions will succeed."})


WORK_ACT_SCHEMA = {
    "name": "work_act",
    "description": "Act in the task marketplace: post a task (title/description/budget), bid on one (taskId/price/message), accept a bid (taskId/bidId), deliver a result (taskId/result), confirm, cancel or dispute (taskId). Escrow and payment are handled by the platform.",
    "parameters": {"type": "object", "properties": {
        "action": {"type": "string", "description": "one of the venue's published actions (see work_observe hint or venue spec)"},
        "taskId": _STR, "title": _STR, "description": _STR,
        "budget": {"type": "number"}, "price": {"type": "number"},
        "message": _STR, "etaHours": {"type": "number"},
        "bidId": _STR, "result": _STR,
    }, "required": ["action"]},
}


def work_act(args: Dict[str, Any], **_: Any) -> str:
    spec = _venue_spec("taskmarket")
    if not spec:
        return _err("taskmarket venue unreachable — try again later")
    ...
    if action not in enum:
        return _err(f"action must be one of {enum}")
    did = C.team_handle()
    path = str(spec.get("routes", {}).get("act", "/agents/{did}/action")).replace("{did}", did)
    body: Dict[str, Any] = {"type": action}
    for k in ("taskId", "title", "description", "budget", "price", "message", "etaHours", "bidId", "result"):
        if args.get(k) is not None:
            body[k] = args[k]
    try:
        r = C.request("POST", path, body, base=_venue_url("taskmarket"), caller_did=did)
    except C.PitchError as e:
        return _err(e.message, status=e.status)
    return _ok({"ok": True, "result": r, "hint": "work_observe again to see the new state."})


TOOLS = TOOLS + (
    ("venues", VENUES_SCHEMA, venues, "🌐"),
    ("work_observe", WORK_OBSERVE_SCHEMA, work_observe, "📥"),
    ("work_act", WORK_ACT_SCHEMA, work_act, "🛠️"),
)
